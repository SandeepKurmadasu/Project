...
# --- Success Cases ---
def test_create_topics_successfully(interactor, topic_storage):
    # Arrange
...
def test_update_topics_successfully(interactor, topic_storage):
    # Arrange
...
def test_get_topics_successfully(interactor, topic_storage):
    # Arrange
    topic_ids = ["T0001"]
    topics = [TopicDTOFactory.build(topic_id="T0001")]
...
def test_not_existing_topic_ids_update_raises(interactor, topic_storage):
    # Arrange
...
def test_not_existing_topic_ids_get_raises(interactor, topic_storage):
    # Arrange
    topic_ids = ["T9999"]
    topic_storage.get_topics_for_topic_ids.return_value = []  # No existing topics
...
def test_not_existed_topic_validate_raises(interactor, topic_storage):
    # Arrange
    topic_id = "T9999"
    ...
    with pytest.raises(NotExistedTopicFound):
        interactor._validate_topic_ids(topic_id)  # Assuming private method access for testing