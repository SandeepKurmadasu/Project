...
from assessment.interactors.attempts_interactor.submit_question import SubmitQuestionInteractor
...
from assessment.interactors.storage_interface.attempt_submitted_questions_storage_interface import AttemptSubmittedQuestionStorageInterface
...
class TestSubmitQuestionInteractor:
    ...
    def _make_mock_answer(self, is_correct_value):
        """Helper to mock EvaluateQuestionInteractor return value"""
        mock_answer = MagicMock()
        mock_answer.is_correct = is_correct_value
        return mock_answer
    ...
    def test_submit_question_correct_answer(self, snapshot, monkeypatch):
        ...
        # mock the evaluate() return object
        mock_answer = self._make_mock_answer(ResponseEnum.CORRECT)
        monkeypatch.setattr(
        ...
        self.attempt_storage.update_assessment_total_points.return_value = self.updated_attempt_correct
        ...
        snapshot.assert_match(repr(result), "submit_correct_answer_snapshot.json")
    ...
    def test_submit_question_wrong_answer(self, snapshot, monkeypatch):
        ...
        mock_answer = self._make_mock_answer(ResponseEnum.WRONG)
        monkeypatch.setattr(
            "assessment.interactors.attempts_interactor.submit_question.EvaluateQuestionInteractor.evaluate",
            MagicMock(return_value=mock_answer)
        )
        ...
        self.question_storage.get_questions.return_value = [self.mock_question]
        self.attempt_storage.update_assessment_total_points.return_value = self.updated_attempt_wrong
        ...
        snapshot.assert_match(repr(result), "submit_wrong_answer_snapshot.json")
    ...
    def test_submit_question_partial_answer(self, snapshot, monkeypatch):
        ...
        mock_answer = self._make_mock_answer(ResponseEnum.PARTIAL_CORRECT)
        monkeypatch.setattr(
            "assessment.interactors.attempts_interactor.submit_question.EvaluateQuestionInteractor.evaluate",
            MagicMock(return_value=mock_answer)
        )
        ...
        self.question_storage.get_questions.return_value = [self.mock_question]
        self.attempt_storage.update_assessment_total_points.return_value = self.updated_attempt_partial
        ...
        snapshot.assert_match(repr(result), "submit_partial_answer_snapshot.json")

    @freeze_time("2024-10-31 10:00:00")
    def test_submit_question_partial_answer_zero_correct_options(self, snapshot, monkeypatch):
        submit_details = SubmitResponseDTO(
            assessment_id="assessment-1234",
            attempt_id="attempt-101",
            question_id="Q1",
        ...
        snapshot.assert_match(repr(result), "submit_partial_zero_correct_options_snapshot.json")
