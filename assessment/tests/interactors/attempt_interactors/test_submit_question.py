...
from cyber_edu_verse.assessment.interactors.attempts_interactor.submit_question import \
    SubmitQuestionInteractor
from cyber_edu_verse.assessment.interactors.dtos import SubmitResponseDTO, \
    AssessmentAttemptDTO, ResponseEnum
from cyber_edu_verse.assessment.interactors.question_dtos import Difficulty
...
class TestSubmitQuestionInteractor:
    ...
    def test_submit_question_correct_answer(self, snapshot, monkeypatch):
        ...
        monkeypatch.setattr(
            "cyber_edu_verse.assessment.interactors.attempts_interactor.submit_question.EvaluateQuestionInteractor.evaluate",
            MagicMock(return_value=ResponseEnum.CORRECT)
        )
        monkeypatch.setattr(
            "cyber_edu_verse.assessment.interactors.attempts_interactor.submit_question.SubmitQuestionInteractor.get_question_scoring",
    ...
    def test_submit_question_wrong_answer(self, snapshot, monkeypatch):
        ...
        monkeypatch.setattr(
            "cyber_edu_verse.assessment.interactors.attempts_interactor.submit_question.EvaluateQuestionInteractor.evaluate",
            MagicMock(return_value=ResponseEnum.WRONG)
        )
        monkeypatch.setattr(
            "cyber_edu_verse.assessment.interactors.attempts_interactor.submit_question.SubmitQuestionInteractor.get_question_scoring",
    ...
    def test_submit_question_partial_answer(self, snapshot, monkeypatch):
        ...
        monkeypatch.setattr(
            "cyber_edu_verse.assessment.interactors.attempts_interactor.submit_question.EvaluateQuestionInteractor.evaluate",
            MagicMock(return_value=ResponseEnum.PARTIAL_CORRECT)
        )
        monkeypatch.setattr(
            "cyber_edu_verse.assessment.interactors.attempts_interactor.submit_question.SubmitQuestionInteractor.get_question_scoring",
    ...
    def test_submit_question_partial_answer_zero_correct_options(self,
                                                                 snapshot,
                                                                 monkeypatch):
        # Test partial answer where correct_options_count == 0 means score should be treated as wrong
        submit_details = SubmitResponseDTO(
            assessment_id="assessment-1234",
            attempt_id="attempt-101",
            question_id="Q1",
