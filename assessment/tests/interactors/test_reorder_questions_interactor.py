...
def interactor(storage):
    return ReorderQuestionsInteractor(question_storage=storage)


def test_reorder_questions_successfully(interactor, storage, snapshot):
    # ARRANGE
    bank_id = "B001"
    ...
    # ✅ Tell storage what to return
    storage.get_question_bank.return_value = existing_bank
    storage.reorder_questions_in_bank.return_value = expected_bank
    ...
    storage.reorder_questions_in_bank.assert_called_once_with(
...
def test_reorder_questions_raises_bank_not_found(interactor, storage, snapshot):
    # ARRANGE
    bank_id = "INVALID_BANK"
    ordered_question_ids = ["Q001", "Q002"]

    storage.get_question_bank.side_effect = QuestionBankNotFound(bank_id)
...
def test_reorder_questions_raises_invalid_order(interactor, storage, snapshot):
    # ARRANGE
    bank_id = "B001"
    ordered_question_ids = ["Q999"]

    storage.reorder_questions_in_bank.side_effect = InvalidQuestionOrder(["Invalid question order"])
