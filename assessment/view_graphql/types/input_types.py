...
class StartAttemptReqParams(graphene.InputObjectType):
    user_id = graphene.String(required=True)
    assessment_id = graphene.String(required=True)

class SubmitQuestionReqParams(graphene.InputObjectType):
    assessment_id = graphene.String(required=True)
    attempt_id = graphene.String(required=True)
    question_id = graphene.String(required=True)
    response = GenericScalar(required=True)

class GetLatestAttemptReqParams(graphene.InputObjectType):
    user_id = graphene.String(required=True)
    assessment_id = graphene.String(required=True)

class GetAttemptProgressReqParams(graphene.InputObjectType):
    attempt_id = graphene.String(required=True)


class GetAttemptScoreReqParams(graphene.InputObjectType):
    attempt_id = graphene.String(required=True)

class GetNextQuestionReqParams(graphene.InputObjectType):
    attempt_id = graphene.String(required=True)

class EndAnAttemptReqParams(graphene.InputObjectType):
    attempt_id = graphene.String(required=True)
