...
from assessment.view_graphql.types.response_types import AutoEndAttemptResponse
from assessment.view_graphql.types.types import AutoEndAttemptType
from course_management.storages.enrollment_storage import EnrollmentStorage


class AutoEndAttemptMutation(graphene.Mutation):
    class Arguments:
        params = EndAnAttemptReqParams(required=True)

    Output = AutoEndAttemptResponse
    ...
    def mutate(root, info, params):
        ...
        interactor = AttemptAutoEndInteractor(attempt_storage=attempt_storage,
                                          assessment_storage=assessment_storage,
                                          enrollment_storage=enrollment_storage)

        try:
            result = interactor.auto_end_attempt(attempt_id=attempt_id)

            return AutoEndAttemptType(
