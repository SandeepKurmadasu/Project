...
from course_management.view_graphql.types.error_types import \
    NotExistingTopicTypes, TopicIdsNotFound
...
from course_management.exceptions.custom_exceptions import \
    NotExistingTopicIdsFound, NotExistingTopicTypesFound
...
def resolve_get_topics(root, info, params):
    try:
        dtos = TopicsInteractor(topic_storage=TopicStorage(),
                                user_storage=UserStorage(),
                                user_learning_unit_storage=UserLearningUnitStorage()).get_topics(
            params.topic_ids)
        topics = [
            TopicType(
                topic_id=d.topic_id,
