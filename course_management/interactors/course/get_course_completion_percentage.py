...
from course_management.interactors.storage_interface.enrollment_storage_interface import \
...
class UserCoursesCompletionPercentageInteractor(ValidationMixIns):
    ...
    def get_user_course_completion_percentage(self, course_id : str ,user_id : str) -> UserLearningPathPercentageDTO:

        self.check_for_user_exists(user_id=user_id, user_storage=self.user_storage)
        self.check_for_course_in_db(course_id=course_id,course_storage=self.course_storage)
